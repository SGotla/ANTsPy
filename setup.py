...
class CMakeBuild(build_ext):
    def run(self):
        ...
        if os.getenv("ITK_DIR"):
            ...
            print(os.path.join(setup_py_dir, "itkbuild/ITKConfig.cmake"))
            os.environ["ITK_DIR"] = os.path.join(setup_py_dir, "itkbuild")
        else:
            print("No local ITK installation found... Building ITK now...")
            subprocess.check_call(["bash", "scripts/configure_ITK.sh"], cwd=setup_py_dir)
            os.environ["ITK_DIR"] = os.path.join(setup_py_dir, "itkbuild")

        ## Find or Configure ANTs ##
        print("Configuring ANTs core")
        print(setup_py_dir)
        subprocess.check_call(["bash", "scripts/configure_ANTsPy.sh"], cwd=setup_py_dir)
        print("done ANTS")
...
setup(
    ...
    package_data={"ants": ["ants/lib/*.pyd*", "lib/*.dll*"]},
